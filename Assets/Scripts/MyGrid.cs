...
public class MyGrid
{
    ...
    public Node GetNeighborWithLowestCostProgressive(int x, int y, int currentNodePathCost)
    {
        ...
        else if (CheckNodeAvailabilityNE(GetNode(x, y)) != TileAvailabilityType.NonExistent && (CheckNodeAvailabilityNE(GetNode(x, y)) != TileAvailabilityType.Blocked && GetNode(x-1, y).DistanceFromSelectedNode== currentNodePathCost - 1 || GetNode(x - 1, y).DistanceFromSelectedNode == 0))
        {
           return GetNode(x-1, y);
        }
        else if (CheckNodeAvailabilitySW(GetNode(x, y)) != TileAvailabilityType.NonExistent && (CheckNodeAvailabilitySW(GetNode(x, y)) != TileAvailabilityType.Blocked && GetNode(x+1, y).DistanceFromSelectedNode== currentNodePathCost - 1 || GetNode(x + 1, y).DistanceFromSelectedNode == 0))
        {
            return GetNode(x + 1, y);
        }
        else if (CheckNodeAvailabilitySE(GetNode(x, y)) != TileAvailabilityType.NonExistent && (CheckNodeAvailabilitySE(GetNode(x, y)) != TileAvailabilityType.Blocked && GetNode(x, y+1).DistanceFromSelectedNode== currentNodePathCost - 1 || GetNode(x, y + 1).DistanceFromSelectedNode == 0))
        {
            return GetNode(x, y +1);
        }
